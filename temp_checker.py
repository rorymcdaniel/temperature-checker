...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("temp_checker.log"), logging.StreamHandler()],
)
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            """,
                (
                    ...
                    weather_data.daily_high,
                    weather_data.daily_low,
                    zip_code,
                ),
            )
            conn.commit()
    ...
    def fetch_weather_data(self, zip_code: str) -> Optional[WeatherData]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                return WeatherData(
                    current_temp=current_temp, daily_high=daily_high, daily_low=daily_low
                )
...
class TemperatureChecker:
    ...
    def should_send_notification(
        self, notification_type: str, app_state: AppState, current_time: Optional[datetime] = None
    ) -> bool:
        ...
        if (
            app_state.last_notification_type == notification_type
            and app_state.last_notification_time
        ):
    ...
    def create_notification_message(
        self, notification_type: str, weather_data: WeatherData, mode: str
    ) -> str:
        ...
        return (
            f"{emoji} <b>{'Close' if notification_type == 'close_windows' else 'Open'} Windows</b>\n\n"
            f"Current temperature: {weather_data.current_temp}°F\n"
            f"Daily high forecast: {weather_data.daily_high}°F\n\n"
            f"{reason}"
        )

    def check_cooling_mode_conditions(
        self, weather_data: WeatherData, app_state: AppState
    ) -> Optional[str]:
        """Check cooling mode conditions and return notification type if needed"""
        # Close windows notification
        if (
            app_state.window_state == "open"
            and weather_data.current_temp >= self.config.close_windows_temp
            and weather_data.daily_high > self.config.forecast_high_threshold
        ):
            return "close_windows"

        # Open windows notification
        if (
            app_state.window_state == "closed"
            and weather_data.current_temp <= self.config.open_windows_temp
        ):
            return "open_windows"

        return None

    def check_heating_mode_conditions(
        self, weather_data: WeatherData, app_state: AppState
    ) -> Optional[str]:
        ...
        if (
            app_state.window_state == "open"
            and weather_data.current_temp <= self.config.heating_close_temp
        ):
            return "close_windows"
        ...
        if (
            app_state.window_state == "closed"
            and weather_data.current_temp >= self.config.heating_open_temp
            and weather_data.daily_high < self.config.heating_forecast_low_threshold
        ):
            return "open_windows"
    ...
    def process_notification(
        self, notification_type: str, weather_data: WeatherData, app_state: AppState
    ) -> bool:
        ...
        success = self.notification.send_message(
            message, self.config.telegram_token, self.config.telegram_chat_id
        )
        ...
        if success:
            ...
            self.database.update_app_state(
                window_state=new_window_state, last_notification_type=notification_type
            )
    ...
    def check_and_notify(self) -> None:
        ...
        logger.info(
            f"Current temp: {weather_data.current_temp}°F, "
